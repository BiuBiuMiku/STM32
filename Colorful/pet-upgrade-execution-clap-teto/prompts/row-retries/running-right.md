Create Codex pet row `running-right` for `execution-clap-teto`: exactly 8 full-body frames in one horizontal strip on flat pure blue #0000FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing chibi humanoid identity, dark red suit, horn-like bows, gloves, bright red eyes, sly grin, proportions, palette, and polished sprite style. This is a v1-to-v2 upgrade; existing standard rows are authoritative.. Preserve silhouette, face, palette, material, proportions, markings, and props.
